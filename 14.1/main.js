...
const maxY = _.maxBy(_.flatten(lines), ([x,y]) => y)[1]
...
function d(){
  ...
  console.log(
    _.map(space.slice(0,10), row => {
      return _.map(row.slice(494,504), b => b ? '.' : '#').join('')
    }).join("\n")
  )
}
...
while(!stopSimulation) {
  ...
  while(!stopSimulation) {
    ...
    else if(sand.x < minX || sand.x > maxX) {
      stopSimulation = true // fell off the side
      break
    }
